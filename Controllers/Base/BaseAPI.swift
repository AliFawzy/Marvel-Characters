...
class BaseAPI {
    ...
    // using GetMoviesList Model
    class func GetCharactersResources(url: String,completion:@escaping(_ status: Int, _ response: characterResources?, _ erorr: ResponseError?) -> Void) {
        
        let timeStamp = "\(getCurrentTimeStamp())"
        let hash = MD5("\(timeStamp)\(Constants.private_key)\(Constants.public_key)")
        let resources_URL = url + "?apikey=\(Constants.public_key)&ts=\(timeStamp)&hash=\(hash ?? "")"

        ApiHandler.performGet(URLString: resources_URL, params: nil, headers: Constants().defaultHeader, authorization: nil) { (status: Int, response: characterResources?, error: ResponseError?) in
